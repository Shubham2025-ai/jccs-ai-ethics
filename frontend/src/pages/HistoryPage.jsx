...
import { Trash2, Eye, Loader, Clock, Shield, Plus, TrendingUp, TrendingDown, Minus, Activity, Layers } from 'lucide-react'
...
export default function HistoryPage() {
  ...
  if (loading) return (
    <div className="flex items-center justify-center min-h-96">
      <Loader className="w-8 h-8 animate-spin text-[#6C63FF]" />
    </div>
  )
    ...
    <div className="space-y-6 py-4 max-w-5xl mx-auto">
      ...
      {audits.length === 0 ? (
        <div className="glass rounded-3xl p-16 text-center border border-white/5 space-y-4">
          ...
          <p className="text-white font-bold text-lg">No evaluations recorded yet</p>
        ...
        <div className="space-y-3">
          {audits.map(a => {
            ...
            return (
              <div key={a.id}
                className="glass rounded-2xl border border-white/5 hover:border-[#6C63FF]/30 transition-all overflow-hidden">
                <div className="flex items-center gap-4 p-4">
                  <ScoreBadge score={a.overall_score} />
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1 flex-wrap">
                      <h3 className="font-bold text-white text-sm truncate">{a.run_name}</h3>
                      {a.target_model_name && (
                    ...
                    <div className="flex items-center gap-3 text-xs text-gray-500 flex-wrap">
                      <span className="flex items-center gap-1"><Layers className="w-3 h-3 text-gray-400" />{a.row_count || 44} Indic Probes</span>
                      <span>·</span>
                      <span className="flex items-center gap-1"><Clock className="w-3 h-3 text-gray-400" />{new Date(a.created_at).toLocaleDateString()}</span>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 flex-shrink-0">
                    ...
                    {a.status === 'completed' && (
                      <Link to={`/results/${a.id}`}
                        className="p-2 rounded-xl transition-all hover:scale-105 bg-[#6C63FF]/20 text-[#a78bfa] border border-[#6C63FF]/30">
                        <Eye className="w-4 h-4" />
                      </Link>
                    )}
                    <button onClick={() => handleDelete(a.id)} disabled={deleting === a.id}
                      className="p-2 rounded-xl transition-all hover:scale-105 bg-red-500/10 text-red-400 border border-red-500/20 disabled:opacity-40">
